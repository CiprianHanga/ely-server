...
struct boss_thaddiusAI : public Scripted_NoMovementAI
{
    ...
    void JustReachedHome() override
    {
        if (m_pInstance)
        {
            ...
            // Respawn Adds:
            Creature* pFeugen = m_pInstance->GetCreature(m_pInstance->GetData64(NPC_FEUGEN));
            Creature* pStalagg = m_pInstance->GetCreature(m_pInstance->GetData64(NPC_STALAGG));
            if (pFeugen)
            {
...
bool EffectDummyNPC_spell_thaddius_encounter(Unit* pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Creature* pCreatureTarget)
{
    ...
    case SPELL_THADIUS_LIGHTNING_VISUAL:
        if (uiEffIndex == EFFECT_INDEX_0 && pCreatureTarget->GetEntry() == NPC_THADDIUS)
        {
            ...
            {
                    pCreatureTarget->AI()->AttackStart(pCreatureTarget->SelectRandomUnfriendlyTarget((Unit*) nullptr,(float) 100));
            }
        }
        return true;
    }
...
struct npc_tesla_coilAI : public Scripted_NoMovementAI
{
    ...
    bool SetupChain()
    {
        ...
        GameObject* pNoxTeslaFeugen = m_pInstance->GetGameObject(m_pInstance->GetData64(GO_CONS_NOX_TESLA_FEUGEN));
        GameObject* pNoxTeslaStalagg = m_pInstance->GetGameObject(m_pInstance->GetData64(GO_CONS_NOX_TESLA_STALAGG));
    ...
    void ReApplyChain(uint32 uiEntry)
    {
        ...
        else                                                // if called from next tick, needed because otherwise the spell doesn't bind
        {
            ...
            GameObject* pGo = m_pInstance->GetGameObject(m_bToFeugen ? m_pInstance->GetData64(GO_CONS_NOX_TESLA_FEUGEN) : m_pInstance->GetData64(GO_CONS_NOX_TESLA_STALAGG));
...
struct boss_thaddiusAddsAI : public ScriptedAI
{
    ...
    Creature* GetOtherAdd()                                 // For Stalagg returns pFeugen, for Feugen returns pStalagg
    {
        ...
        {
        case NPC_FEUGEN:  return m_pInstance->GetCreature(m_pInstance->GetData64(NPC_STALAGG));
        case NPC_STALAGG: return m_pInstance->GetCreature(m_pInstance->GetData64(NPC_FEUGEN));
        default:
            return NULL;
        }
    }
...
struct boss_stalaggAI : public boss_thaddiusAddsAI
{
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        if (m_pInstance->GetCreature(m_pInstance->GetData64(NPC_FEUGEN))->GetHealth()<1000)
            m_pInstance->GetCreature(m_pInstance->GetData64(NPC_THADDIUS))->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
    }
...
struct boss_feugenAI : public boss_thaddiusAddsAI
{
    ...
    void JustDied(Unit* /*pKiller*/) override
    {
        ...
        if (m_pInstance->GetCreature(m_pInstance->GetData64(NPC_STALAGG))->GetHealth()<1000)
            m_pInstance->GetCreature(m_pInstance->GetData64(NPC_THADDIUS))->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
    }
